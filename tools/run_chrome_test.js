...
(async () => {
  ...
  // Diagnostic: print current body HTML (helps check whether the income input was created)
  const bodyHTML = await page.evaluate(() => document.body.innerHTML);
  console.log('PAGE BODY AFTER addIncomeSource:\n', bodyHTML.slice(0, 2000));

  // Wait for input to appear
  await page.waitForSelector('.income-item input[type="number"]');
  ...
  // screenshot
  const outPath = path.join(root, 'tools', 'taxcortex_result.png');
  await page.screenshot({ path: outPath, fullPage: true });
  console.log('Screenshot saved to', outPath);

  // Keep the browser open for a short time so you can see the interaction (8s), then close
  console.log('Browser will remain open for 8s so you can view the interaction...');
